...
async def test_semantic_search():
    ...
    with (
        patch("app.tools.search.get_client", return_value=mock_client),
...
async def test_semantic_search_end_to_end():
    ...
    mock_client.get_opinion_full_text.side_effect = [f"Full text for case {i}" for i in range(5)]
...
def test_vector_store_lazy_initialization():
    ...
    with patch("app.tools.search._vector_store_instance", None):
        ...
        with patch(
            "app.analysis.search.vector_store.LegalVectorStore", return_value=mock_store_instance
        ):
            # Reset global
